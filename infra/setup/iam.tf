...
resource "aws_iam_policy" "terraform_backend_policy" {
  name        = "${aws_iam_user.cd.name}-terraform-backend-policy"
  description = "Allows user to access S3 and DynamoDB for Terraform backend"
  policy      = data.aws_iam_policy_document.terraform_backend_policy.json
}

resource "aws_iam_user_policy_attachment" "terraform_backend_policy_attachment" {
  user       = aws_iam_user.cd.name
